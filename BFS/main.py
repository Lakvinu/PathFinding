    ...
    while myde:
        ...
        for i in range(4):
            ...
            if (new_y, new_x) == end_place:
                return cur + [(new_y, new_x)]

            if new_y in y_possible and new_x in x_possible:
                if new_grid[new_y][new_x] == '.':
                    new_grid[new_y][new_x] = '1'
                    posible_moves.append((new_y, new_x))
                    #myde.append(cur + [(new_y, new_x)])

        new_order = sort_arr(posible_moves, len(posible_moves), end_place)

        for i in range(len(posible_moves)):

            new_row, new_col = new_order[i][1]
            grid[new_row][new_col]['bg'] = 'red'
            myde.append(cur + [new_order[i][1]])
