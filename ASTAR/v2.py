...
def sort_arr(arr, start, p):
    ...
    for i in range(len(arr)):
        ...
        end_dist = pow((p[0] - arr[i][0][-1][0]), 2) + pow((p[1] - arr[i][0][-1][1]), 2)

        # start_dist = abs(start[0] - arr[i][0][-1][0]) + abs(start[1] - arr[i][0][-1][1])

        start_dist = pow((arr[i][0][-1][0] - arr[i][0][-2][0]), 2) + pow((arr[i][0][-1][1] - arr[i][0][-2][1]), 2)

        # vp.append([end_dist + start_dist, arr[i]])
        arr[i][1] = start_dist ** 0.5 + (end_dist) ** 0.5 + cur_distance
...
def update_distance(dis, end_place, row, col):
    ...
    if dis != 0:
        print(dis)
        dis -= (pow(end_place[0] - row, 2) + pow(end_place[1] - col, 2)) ** 0.5
